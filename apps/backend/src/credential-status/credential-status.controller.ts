...
import { ApiOperation, ApiTags, ApiExcludeEndpoint, ApiParam } from '@nestjs/swagger';
...
export class CredentialStatusController {
    ...
    async getStatusList(@Param('id') id: string) {
        try {
            if (!this.isValidUUID(id)) {
                throw new BadRequestException('Invalid status list ID format. Must be a valid UUID.');
            }

            const statusList = await this.statusService.getStatusList(id);
            if (!statusList) {
                throw new NotFoundException(`Status list with ID ${id} not found`);
            }
            return statusList;
        } catch (error) {
    ...
    async revokeCredential(@Param('credentialId') credentialId: string) {
        try {
            ...
            const isValid = await this.statusService.verifyCredentialStatus(credentialId);
    ...
    async createTestVC() {
        try {
            ...
            const schemaTemplate = await this.schemaTemplateService.create({
                name: `Schema_StudentIDCard_${timestamp}`,
                version: "1.0",
                schema: {
                    "@context": [
            ...
            const { signedCredential, credential } = await this.vcService.issueVerifiableCredential(vc.id);
    ...
    async decodeStatusList(@Param('statusListId') statusListId: string) {
        try {
            ...
            const statusList = await this.statusService.getStatusList(statusListId);
            if (!statusList) {
                throw new NotFoundException(`Status list with ID ${statusListId} not found`);
            }

            // 2. Extract encodedList
            const encodedList = statusList.credentialSubject.encodedList;
            if (!encodedList) {
                throw new BadRequestException('No encodedList found in status list');
            }

            // 3. Fix base64 padding if needed
            const fixedBase64 = encodedList.replace(/-/g, '+').replace(/_/g, '/');
            ...
            return {
                ...
                decodedList: {
                    ...
                    summary: {
                        ...
                        revoked: Array.from(bitArray).filter(bit => bit === 1).length,
                        valid: Array.from(bitArray).filter(bit => bit === 0).length
                    }
                },
                debug: {
