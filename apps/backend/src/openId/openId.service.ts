...
export function getOpenIdConfigMetadata(
    baseUrl: string,
    presentationDefinitions: PresentationDefinition[],
): OpenIdConfiguration {
    const scopesSupported = presentationDefinitions.map((def) => def.scope);
    return {
        ...
        scopes_supported: ["openid", ...scopesSupported],
        ...
        presentation_definitions_supported: presentationDefinitions.map(
            (def) => ({
                id: def.definition.id,
                format: def.definition.format,
                constraints: def.definition.input_descriptors.map(
                    (descriptor) => ({
                        id: descriptor.id,
...
export async function getOpenIdIssuerMetadata(
    baseUrl: string,
): Promise<OpenIdIssuer> {
    return {
...
export class OpenIDProviderService {
    ...
    private async initializeProvider() {
        ...
        const credentialsSupported =
            await this.schemaTemplateService.getCredentialsSupported();

        console.log({ credentialsSupported });
        const presentationDefinitions =
            await this.presentationDefinitionService.findAll();
        ...
        const configMetadata = getOpenIdConfigMetadata(
            HOST,
            presentationDefinitions,
        );

        console.log({ configMetadata });
        console.log({ presentationDefinitions });

        if (
            !process.env.ISSUER_PRIVATE_KEY_JWK ||
            !process.env.ISSUER_PRIVATE_KEY_ID ||
            !process.env.ISSUER_PUBLIC_KEY_JWK
        ) {
            throw new Error(
                "Missing required environment variables for issuer",
            );
        }
