...
export class ScopeCredentialMappingService {
    ...
    async create(
        presentationDefinitionId: number,
        credentialSchemaId: number,
    ): Promise<ScopeCredentialMapping> {
        const presentationDefinition =
            await this.presentationDefinitionRepo.findOne({
                where: { id: presentationDefinitionId },
            });
        if (!presentationDefinition) {
            throw new NotFoundException(
                `PresentationDefinition with ID ${presentationDefinitionId} not found`,
            );
        }

        const credentialSchema = await this.credentialSchemaRepo.findOne({
            where: { id: credentialSchemaId },
        });
        if (!credentialSchema) {
            throw new NotFoundException(
                `CredentialSchema with ID ${credentialSchemaId} not found`,
            );
        }
        ...
        if (credentialSchema.templateVariables.length > 0) {
            throw new ConflictException(
                `CredentialSchema with ID ${credentialSchemaId} has template variables. Template variables are not supported in scope mappings.`,
            );
        }

        const existingMapping = await this.mappingRepo.findOne({
            where: { presentationDefinition: { id: presentationDefinitionId } },
        });
        if (existingMapping) {
            throw new ConflictException(
                `A schema is already bound to the scope with PresentationDefinition ID ${presentationDefinitionId}`,
            );
        }
    ...
    async findAllMappings(): Promise<ScopeCredentialMapping[]> {
        return this.mappingRepo.find({
            relations: ["presentationDefinition", "credentialSchema"],
        });
    }
    ...
    async getCredentialSchemaByScope(scope: string): Promise<CredentialSchema> {
        ...
        if (!presentationDefinition) {
            throw new Error(
                `PresentationDefinition with scope ${scope} not found`,
            );
        }
        ...
        const mapping = await this.mappingRepo.findOne({
            where: {
                presentationDefinition: { id: presentationDefinition.id },
            },
