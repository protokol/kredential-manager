...
import { DidEntityType, FormatType, JWK, JWTHeader, KeyType } from "./types";
...
export class Ebsi {
  ...
  createLegalEntityDid(): Promise<any> {
    return this.rpcClient.call("createDid", ["LEGAL_ENTITY"]);
  }
  ...
  createNaturalPersonDid(publicKeyJwk?: JWK): Promise<any> {
    return this.rpcClient.call(
      "createDid",
      publicKeyJwk ? ["NATURAL_PERSON", publicKeyJwk] : ["NATURAL_PERSON", ""],
    );
  }

  // Generate Key Pair
  async generateKeyPair(type: KeyType): Promise<any> {
    return this.rpcClient.call("generateKeyPair", [type]);
  }

  // Get Public Key
  async getPublicKey(privateKey: any, format: FormatType): Promise<any> {
    return this.rpcClient.call("getPublicKey", [privateKey, format]);
  }

  // Format Public Key
  async formatPrivateKey(privateKey: any, format: FormatType): Promise<any> {
    return this.rpcClient.call("formatPrivateKey", [privateKey, format]);
  }

  // Format Public Key
  async formatPublicKey(publicKey: any, format: FormatType): Promise<any> {
    return this.rpcClient.call("formatPublicKey", [publicKey, format]);
  }

  // Get Ethereum Address
  async getEthereumAddress(privateKey: string): Promise<any> {
    return this.rpcClient.call("getEthereumAddress", [privateKey]);
  }
